...
│       ├── display/           # 屏幕显示
│       │   ├── display.c/.h       # 屏幕抽象层（当前为日志模拟，待接入 TFT）
│       │   └── layout_renderer.c/.h # 按配置把数据渲染到屏幕
│       └── web/               # 内嵌前端资源
...
└── web/               # 网页管理界面（React + TS + Vite + Ant Design）
    └── src/
        ...
        ├── pages/ApiConfig.tsx       # 接口配置 + 测试
        ├── pages/FieldSelect.tsx     # 数据字段解析/选择
        └── pages/ScreenLayout.tsx    # 屏幕显示配置（网格布局）
```
...
3. **配置接口**：在「接口配置」输入股票数据接口地址，点击「测试接口」，设备自动解析返回的 JSON 字段。
4. **选择字段**：在「字段解析」勾选要显示的字段，添加到显示列表。
5. **布局屏幕**：在「屏幕布局」调整每个字段的位置、大小、字号、小数位、涨跌颜色，保存。
6. **自动盯盘**：设备按配置的刷新频率自动获取数据并刷新屏幕。
...
- [ ] 接入真实 LCD/TFT 驱动（替换 `display.c` 模拟实现）
- [ ] 拖拽式布局交互（当前为网格参数配置）
- [ ] 字段格式化更多规则（格式化类型选择控件）
- [ ] 多接口支持
